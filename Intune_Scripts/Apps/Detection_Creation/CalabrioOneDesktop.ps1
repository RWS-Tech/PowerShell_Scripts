...
$32BitPath = "${Env:ProgramFiles(x86)}\Calabrio One\Desktop\active\bin\DCC.exe"

# Check for Application (File Detection Method)
$AppExe = (Get-ChildItem -Path $32BitPath -ErrorAction SilentlyContinue)
$AppExe.FullName
$AppPath = $($AppExe.FullName).Replace("${Env:ProgramFiles(x86)}\","")
